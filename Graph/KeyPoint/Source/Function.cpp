...
void MGraph::DFSArticul(int cur)
{
	int iMin = visited[cur] = ++count;
	int iPosNext = 0;
	for (iPosNext = FirstAdjVex(vexs[cur]); iPosNext >= 0; iPosNext = NextAdjVex(vexs[cur], vexs[iPosNext])) {
		if (visited[iPosNext] == 0) {
			DFSArticul(iPosNext);
			if (low[iPosNext] >= visited[cur]) {
				cout << "Key Point:" << cur << ends << vexs[cur] << endl;
			}
			else if (low[iPosNext] < iMin) {
				iMin = low[iPosNext];
			}
		}
		else if(visited[iPosNext]<iMin){
			iMin = visited[iPosNext];
		}
	}
	low[cur] = iMin;
	LowOrder[cur] = ++LowCount;
}
void MGraph::FindArticul()
{
	...
	low[0] = 0;
	LowOrder[0] = 0;
	DFSArticul(FirstAdjVex(vexs[0]));
	if (count < arcnum) {
		int iPosNext = 0;
		cout << "Key Ponit:" << 0 << ends << vexs[0] << endl;
		for (iPosNext = FirstAdjVex(vexs[0]); iPosNext >= 0; iPosNext = NextAdjVex(vexs[0], vexs[iPosNext])) {
			if (visited[iPosNext] == 0) {
				DFSArticul(iPosNext);
			}
		}
...
void MGraph::DisplayInfo()
{
	for (int i = 0; i < vexnum; i++) {
		cout << i << ends << ends <<vexs[i] << ends <<ends << visited[i] << ends <<ends << low[i] << ends <<ends << LowOrder[i] << endl;
	}
}
