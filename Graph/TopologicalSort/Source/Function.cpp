...
Status cALGraph::TopologicalSort()
{
	...
	while (StackEmpty(S) != TRUE) {
		//SElemType*pelem = NULL;
		//for (pelem = S.base; pelem < S.top; pelem++) {
		//	cout << *pelem << ends;
		//}
		//cout << endl;
		Pop(S, elem);

		//pNode = vexs[elem].firstarc;
		cout << vexs[elem].data << ends;
		iOutput++;
		for (int iAdj = FirstAdjVex(vexs[elem].data); iAdj >= 0; iAdj = NextAdjVex(vexs[elem].data, vexs[iAdj].data)) {
			indegree[iAdj]--;
			if (indegree[iAdj] == 0) {
				Push(S, iAdj);
			}
	...
	if (iOutput < vexnum) {
		cout << "There is loop in the graph." << endl;
		return ERROR;
	}
	else {
		cout << "This is one kind of sorts." << endl;
		return OK;
	}
}
void cALGraph::FindInDegree()
{
	...
	for (int i = 0; i < vexnum; i++) {
		pNode = vexs[i].firstarc;
		while (pNode != NULL) {
			indegree[pNode->data.adjvax]++;
			pNode = pNode->nextarc;
		}
	}
}
