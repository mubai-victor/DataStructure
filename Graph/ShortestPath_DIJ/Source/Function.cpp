...
void MGraph::ShortestPath_DIJ(int v)
{
	int iMin = 0, iPreVex = 0;
	bool bResult[MAX_VERTEX_NUM];
	for (int i = 0; i < vexnum; i++) {
		bResult[i] = FALSE;
		distance[i] = arcs[v][i].adj;
		for (int j = 0; j < vexnum; j++) {
			path[i][j] = FALSE;
		}
		if (distance[i] < INT_MAX) {
			path[i][i] = path[i][v] = TRUE;
		}
	}
	bResult[v] = TRUE;
	distance[v] = 0;
	...
	for (int j = 0; j < vexnum; j++) {
		iMin = INT_MAX;
		for (int i = 0; i < vexnum; i++) {
			if (bResult[i] == FALSE&&distance[i] < iMin) {
				iMin = distance[i];
				iPreVex = i;
			}
		}
		bResult[iPreVex] = TRUE;
//		path[iPreVex][iPreVex] = TRUE;
		for (int k = 0; k < vexnum; k++) {
			if ((bResult[k] == FALSE)&&(arcs[iPreVex][k].adj < INT_MAX)&&(iMin + arcs[iPreVex][k].adj < distance[k]) && (iMin < INT_MAX)) {
				distance[k] = iMin + arcs[iPreVex][k].adj;
				for (int i = 0; i < vexnum; i++) {
					path[k][i] = path[iPreVex][i];
				}
				path[k][k] = TRUE;
			}
		}
...
void MGraph::PrintInfo(int v)
{
	cout << "The path and distances are:" << endl;
	for (int i = 0; i < vexnum; i++) {
		//if (i == v) {
		//	continue;
		//}
