...
{
	int iPos = 0;
	VRType weight = INT_MAX;
	for (int i = 0; i < vexnum; i++) {
		if (min[i].lowcost > 0 && min[i].lowcost < weight) {
			iPos = i;
			weight = min[i].lowcost;
		}
	}
	return iPos;
}
void MGraph::MiniSpanTreePrim()
{
	int iAddElem = 0;
	for (int i = 0; i < vexnum; i++) {
		min[i].lowcost = arcs[0][i].adj;
		strcpy_s(min[i].adjvex, vexs[0]);
	}
	min[0].lowcost = 0;
	for (int i = 1; i < vexnum; i++) {
		iAddElem = MinNum();
		cout << "The new arc:" << vexs[iAddElem] << "->" << min[iAddElem].adjvex << endl;
		min[iAddElem].lowcost = 0;
		for (int j = 1; j < vexnum; j++) {
			if (arcs[iAddElem][j].adj < min[j].lowcost) {
				min[j].lowcost = arcs[iAddElem][j].adj;
				strcpy_s(min[j].adjvex, vexs[iAddElem]);
			}
		}
	}
}
void MGraph::Kruskal()
{
	int iSet[MAX_VERTEX_NUM];
	int iMin = INT_MAX, iPosx = 0, iPosy = 0;
	for (int i = 0; i < vexnum; i++) {
		iSet[i] = i;
	}
	for (int k = 0; k < vexnum - 1; k++) {
		...
		iMin = arcs[iPosx][iPosy].adj = INT_MAX;
		if (iSet[iPosx] != iSet[iPosy]) {
			cout << "The new arc:" << vexs[iPosx] << "->" << vexs[iPosy] << endl;
			for (int i = 0; i < vexnum; i++) {
				if (iSet[i] == iSet[iPosx]) {
					iSet[i] = iSet[iPosy];
				}
			}
		}
