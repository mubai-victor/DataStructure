...
Status cALGraph::TopologicalSort()
{
	SqStack S;
	int iPosCenter = 0, iPosNext = 0;
	int iCount = 0;
	ArcNode *pNode = NULL;
	InitStack(S);
	InitStack(T);
	...
	while (StackEmpty(S) != TRUE) {
		Pop(S, iPosCenter);
		Push(T, iPosCenter);
		cout << vexs[iPosCenter].data << ends;
		iCount++;
		for (iPosNext = FirstAdjVex(vexs[iPosCenter].data); iPosNext >= 0; iPosNext = NextAdjVex(vexs[iPosCenter].data, vexs[iPosNext].data)) {
			indegree[iPosNext]--;
			if (indegree[iPosNext] == 0) {
				Push(S, iPosNext);
			}			
		}
		for (pNode = vexs[iPosCenter].firstarc; pNode != NULL; pNode = pNode->nextarc) {
			if (ve[iPosCenter] + *pNode->data.info > ve[pNode->data.adjvax]) {
				ve[pNode->data.adjvax] = ve[iPosCenter] + *pNode->data.info;
			}
		}
	}
	cout << endl;
	if (iCount == vexnum) {
		cout << "This is a kind of sort." << endl;
		return OK;
	}
	else {
		cout << "There is(are) the loop(s) in the net." << endl;
		return ERROR;
	}
}
void cALGraph::FindInDegree()
{
	ArcNode *pNode = NULL;
	int iPosNext = 0;
	for (int i = 0; i < vexnum; i++) {
	...
	for (int i = 0; i < vexnum; i++) {
		for (iPosNext = FirstAdjVex(vexs[i].data); iPosNext >= 0; iPosNext = NextAdjVex(vexs[i].data, vexs[iPosNext].data)) {
			indegree[iPosNext]++;
		}
	}
}
Status cALGraph::CriticalPath()
{
	int iMax = 0;
	ArcNode *pNode = NULL;
	int iPosCenter = 0;
	int ee = 0, el = 0;
	if (TopologicalSort()==ERROR) {
		return ERROR;
	}
	//for (int i = 0; i < vexnum; i++) {
	//	if (ve[i] > iMax) {
	...
	while (StackEmpty(T) != TRUE) {
		Pop(T, iPosCenter);
		for (pNode = vexs[iPosCenter].firstarc; pNode != NULL; pNode = pNode->nextarc) {
			if (vl[pNode->data.adjvax] - *pNode->data.info < vl[iPosCenter]) {
				vl[iPosCenter] = vl[pNode->data.adjvax] - *pNode->data.info;
			}
		}
	}
	for (int i = 0; i < vexnum; i++) {
		if (vl[i] == ve[i]) {
			cout << "Point in the critical path." << vexs[i].data << endl;
		}
	}
	for (int i = 0; i < vexnum; i++) {
		for (pNode = vexs[i].firstarc; pNode != NULL; pNode = pNode->nextarc) {
			ee = ve[i];
			el = vl[pNode->data.adjvax] - *pNode->data.info;
			cout << vexs[i].data << "->" << vexs[pNode->data.adjvax].data << " ee:" << ee << " el:" << el;
			if (ee == el) {
...
int cALGraph::FirstAdjVex(VertexType vex)
{
	...
	if (iPos >= 0) {
		ElemType elem;
		if (GetElem(vexs[iPos].firstarc, 1, elem) == OK) {
			return elem.adjvax;
		}
	}
	return -1;
...
int cALGraph::NextAdjVex(VertexType center, VertexType pre)
{
	...
	return LPre->nextarc->data.adjvax;
}
