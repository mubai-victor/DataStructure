...
{
	DuLinkList p_front, p_back = L->next;
	while (p_back != L){
		p_front = p_back;
		p_back = p_back->next;
		free(p_front);
...
{
	DuLinkList p_front, p_back = L->next->next;
	while (p_back != L){
		if (p_back->data == elem_cur){
			elem_pri = p_back->prior->data;
			return OK;
		}
		p_back = p_back->next;
	}
...
Status ListInsert(DuLinkList&L, int i, Elemtype elem){
	if (i <= 0 || i > ListLength(L) + 1)
		return ERROR;
	DuLinkList p = L,p_temp;
	int count = 1;
	while (count < i&&p->next != L){
		count++;
		p = p->next;
	}
	p_temp = (DuLinkList)malloc(LEN);
	p_temp->data = elem;
	p_temp->next = p->next;
	p->next->prior = p_temp;
	p->next = p_temp;
