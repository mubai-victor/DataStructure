...
//		printf("1.%d,%d\n", ev_pre.NType, ev_pre.OccurTime);
	...
	printf("第%d位客户在%d时刻到来，在窗口%d等待。\n", CustomerNum, ev_pre.OccurTime, i);
//	ListTraverse(evlist, Print);
	EnQueue(q[i], q_elem);
//	ListTraverse(evlist, Print);
...
{
	QElemType q_elem;
	DeQueue(q[ev_pre.NType], q_elem);
	TotalTime += ev_pre.OccurTime - q_elem.ArrivalTime;
	printf("客户在%d时刻从窗口%d离开。\n",ev_pre.OccurTime, ev_pre.NType+1);
	if (!QueueEmpty(q[ev_pre.NType])) {
		GetHead(q[ev_pre.NType],q_elem);
		ev_temp.OccurTime = ev_pre.OccurTime + q_elem.Duration;
