...
void WordListType::ExtractKeyword(int &BookNo)//将书名中的关键词提取到item中去，BookNo是书的检索号码；
{
	...
	while (bFlag) {
		...
		if (head[0] >= 'A'&&head[0] <= 'Z') {
			for (loop = 0; loop < noidx.length&&strcmp(noidx.item[loop], head); loop++);
			if (loop < noidx.length) {
...
{	
	int status = 0;
	for (pos = 0; (status = item[pos].keyword.StrCompare(wd) < 0); pos++);
	if (status > 0) {
		pos = pos - 1;//如果status大于0，说明当前位置的值在字母表中排在item[pos].keyword的后边，所以该位置的前一个位置才是插入位置
		return FALSE;
	}
	else {
		return TRUE;	
...
void IdxListType::InsertNewKey(int i, SString wd)//在检索表上i位置插入关键词wd
{
	for (int j = length - 1; j >= i - 1; j--) {
		item[j + 1].keyword.StrCompare(item[j].keyword);
	}
	item[i - 1].keyword.StrCopy(wd);
	length++;
}
...
{
	Link l;
	fprintf(p, "%d/n", length);
	for (int i = 0; i < length; i++) {
		for (int j = 0; j < item[i].keyword.StrLength(); j++) {
			fprintf(p, "%c", item[i].keyword.str[i]);
		}
